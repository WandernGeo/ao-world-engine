...
-- Initialize codec loader (registers LoadCodec AO handler)
local codec = require("codec_loader")
codec.register_handler()

-- Load NPC module (thin codec loader — data arrives via LoadCodec messages)
-- Actual NPC data is loaded from Arweave codecs:
--   codec_01_npcs → founding NPCs
--   codec_npcs_generated → 800 generated NPCs
--   codec_16_buildings → building registry
local npc_data = require("all_npcs")
ALL_NPCS = npc_data.ALL_NPCS or {}
BUILDINGS = npc_data.BUILDINGS or {}
...
function Initialize(custom_config)
    ...
    -- Load founding NPCs
    print("5. Loading Founding NPCs...")
    FoundingNPCs = {}
    local ok, founding_module = pcall(require, "founding_npcs")
    if ok and founding_module and founding_module.FOUNDING_NPCS then
        FoundingNPCs = founding_module.FOUNDING_NPCS
        local founder_count = 0
        for key, npc in pairs(FoundingNPCs) do
            founder_count = founder_count + 1
            print("   - " .. npc.name .. " (" .. npc.faction .. ")")
        end
        print("   Total Founders: " .. founder_count)
    else
        print("   - Warning: founding_npcs.lua not loaded (will load via codec)")
    end
    
    -- Report codec-loaded NPCs
    local codec_npc_count = codec.count_keys(ALL_NPCS)
    if codec_npc_count > 0 then
        print("   Codec-loaded NPCs: " .. codec_npc_count)
    else
        print("   - NPCs pending: send LoadCodec messages to populate")
    end
    print("")
    
    -- Register with Layer Registry (Multiverse)
    print("6. Registering with Layer Registry...")
    LAYER_ID = config.layer_id or "layer_00_testnet"
    LAYER_REGISTRY = config.layer_registry or nil
    ...
    if LAYER_REGISTRY then
        ao.send({
            Target = LAYER_REGISTRY,
            Action = "register-layer",
            Data = json.encode({
                layer_id = LAYER_ID,
                name = "RE:ECHO City Testnet",
                population = total_npcs,
                geo_claim = geo_claim,
                status = "active"
            })
        })
        print("   - Registered as: " .. LAYER_ID)
        print("   - Geo-claim: NYC")
    else
        print("   - Standalone mode (no registry)")
    ...
    print("  ")
    print("  Next steps:")
    print("  1. Load codecs via LoadCodec messages")
    print("  2. Spawn district processes")
    print("  3. Register districts with world")
    print("  4. Start CRON for tick advancement")
    print("==============================================")
